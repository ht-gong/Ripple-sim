...
class Routing:
	def __init__(self):
		...
		#self.SLICE_E = 5000	# length of active slot, unit: nanosecond
		#self.SLICE_R = 1000 	# reconfiguration time, unit: nanosecond
		...
		self.last_tors = [] 		# last_tors contains many lists: [tor_id, ddl]
		# intem_tors contains many lists: [layer_id, tor_id, tor_times, ddl]
		self.intem_tors = []
		# pre_tor: record each tor's predecessor
		# key = (layer_id, tor_id, tor_times), value = (predecessor_tor_id, tor_times)
		self.pre_tor = {}
	...
	def is_loop(self, tor_lower_layer, tor_crt_layer, tor_times_crt_layer, layer_id):
		...
		while (True):
			if layer_id == 1:
				break
			[tor_upper_layer, tor_times_upper_layer] = self.pre_tor[(layer_id, tor_crt_layer, tor_times_crt_layer)]
	...
	# SLICE_E >= TRANS_DELAY. SLICE_E should transfer at least one packet!
	def cal_early_late_sending(self, slot_idx, s, d):
		...
		if self.SLICE_R > self.TRANS_DELAY:
			early_send = begin
			if is_next_1:
				late_send = end
			else:
			...
			if slot_idx % 2 == 0:		# SLICE_E
				...
				if is_next_1:
					if is_next_2:		# E R E
						...
						late_send = end + self.SLICE_R - self.TRANS_DELAY
				else:					# E 
					late_send = end - self.TRANS_DELAY
			else:		# SLICE_R
				if is_next_1:			# R E
	...
	def add_tor_next_layer(self, layer_id, tor_next_layer, tor_crt_layer, tor_times_crt_layer, new_ddl_timing):
		...
		self.pre_tor[(layer_id+1, tor_next_layer, tor_times_next_layer)] = (tor_crt_layer, tor_times_crt_layer)
	...
	def get_optimal_paths(self):
		...
		for i in range(len(self.last_tors)):
			...
			self.add_tor_next_layer(0, la_tor, self.dst_tor, 1, ddl_dst_timing)
			
			if ddl_dst > self.crt_time:
				break
				#pass
			...
			while(True):
				...
				if 0 <= early_sending <= exactly_sending:
					self.cout_op_path(tor_crt_layer, tor_times_crt_layer, layer_id, ddl_dst)
	...
	def cout_op_path(self, tor_crt_layer, tor_times_crt_layer, layer_id, ddl_dst):
	...
	def write_optimal_paths_many_files(self):
		...
		self.file_for_one_path.write(str(self.one_path) + '\n')
		self.file_for_all_paths.write(str(self.src_tor) + ' ' + str(self.dst_tor) + '\n')
		for item in self.optimal_paths:
			self.file_for_all_paths.write(str(item) + '\t')
...
if __name__ == '__main__':
	...
	#route.src_tor = 0
	#route.dst_tor = 1
	#print(route.cal_slice_begin_end(194))
	#route.get_optimal_paths()

	for start_time in range(107, 108):
	...
	'''
	for start_time in range(route.N):
		for i in range(route.N):
			for j in range(route.N):
				if i == j:
					continue
				print('*******************************************')
				print(i, j)
				route.get_optimal_paths(start_time, i, j)
				route.get_direct_paths(start_time, i, j)
	
	route.write_optimal_paths()
	route.multihop_file.close()
	'''