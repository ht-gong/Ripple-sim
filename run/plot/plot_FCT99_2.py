...
for fname, label in files:
    ...
    with open(f"../sim/{sys.argv[1]}/{fname}", "r") as f:
        lines = f.readlines()
        for line in lines:
            line = line.split()
            if len(line) <= 0:
    ...
    a, = ax1.plot(flowsizes, fcts99, marker='+', label=label, linestyle='-')
    ...
    a, = ax2.plot(flowsizes, fcts50, marker='+', label=label, linestyle='-')
...
plt.savefig(f"./{dir}/plot_{sys.argv[1]}.png", bbox_inches='tight')
plt.show()
plt.close()
