...
%%%%%%%%%%%%%%%%%%%%%% skewness: x portion ToRs carry y traffic
%%%%%%%%%%%%%%%%%%%%%% x = 0.5, y = 0.5 ====> uniform traffic
x = 0.1;
y = 0.9;

filename=sprintf('websearch_%.0fpercLoad_%.0fsec_%dhosts_skew',100*loadfrac0,totaltime,H);
...
% websearch workload:
data=csvread('../../traffic/websearch.csv');
flowsize=data(:,1).';
flowcdf=data(:,2).';
...
num_ToR_pairs = Nrack * (Nrack - 1);
ToR_pairs = zeros(num_ToR_pairs, 2);
index = 0;
for src_ToR = 0:Nrack-1
    for dst_ToR = 0:Nrack-1
        if src_ToR ~= dst_ToR
            index = index + 1;
            ToR_pairs(index, :) = [src_ToR, dst_ToR];
        end
    end
end
ToR_pair_indices = 1:num_ToR_pairs;

skew_ToR_pairs = randsample(ToR_pair_indices, ceil(x * num_ToR_pairs));
non_skew_ToR_pairs = setdiff(ToR_pair_indices, skew_ToR_pairs);
...
for a=1:nflows
    ...
    if randvect(a) < y
        index = randsample(skew_ToR_pairs, 1);
    else
        index = randsample(non_skew_ToR_pairs, 1);
    end
    flowmat1(a, 1) = ToR_pairs(index, 1) * 6 + randi(6);
    flowmat1(a, 2) = ToR_pairs(index, 2) * 6 + randi(6);
end
