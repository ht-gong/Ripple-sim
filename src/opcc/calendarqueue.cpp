...
void CompositeQueue::completeService() {
    ...
    int crt = _top->is_downlink(_port) ? _dl_queue : _sending_pkt->get_crtslice();
    ...
    if (!_enqueued_high[crt].empty() || !_enqueued_low[crt].empty() || !_enqueued_high_prime[crt].empty() || !_enqueued_low_prime[crt].empty())
        beginService();
}
...
void CompositeQueue::clearSliceQueue(int slice){
    ...
    if(high_drop + low_drop > 0)
        cout << "Calendar at " << _tor << _port << " changed dropping slice " << slice << " high,low " << high_drop << "," << low_drop << endl;
...
mem_b CompositeQueue::slice_maxqueuesize(int slice){
    ...
    if (slice == _dl_queue){
        ...
        //FD I need the duration of the slice, is this right?
...
mem_b CompositeQueue::slice_queuesize(int slice){
    assert(slice <= _dl_queue);
    int  s =  _queuesize_high[slice]+_queuesize_low[slice]
        +_queuesize_high_prime[slice]+_queuesize_low_prime[slice];
    //if (slice == _dl_queue) cout << "dl size " << s << endl;
...
void QueueAlarm::doNextEvent(){
    ...
    if(_queue->slice_queuesize(crt) > 0 && _queue->_serv == QUEUE_INVALID){
