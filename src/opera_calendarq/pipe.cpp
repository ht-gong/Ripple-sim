  ...
  : EventSource(eventlist,"utilmonitor"), _top(top)
{
    ...
    uint64_t rate = LINK_RATE / 8; // bytes / second
    rate = rate * _H;
    ...
    if (_counter % REPORTING_PERIOD == 0) {
        ...
        for (int tor = 0; tor < _N; tor++) {
            for (int downlink = 0; downlink < _hpr; downlink++) {
                ...
                cout << "Pipe " <<  tor << " " << downlink << " " << (double) bytes / (LINK_RATE / 8 * timeAsSec(_period) * REPORTING_PERIOD)  << endl;
            }
            for (int uplink = 0; uplink < _hpr; uplink++) {
                ...
                cout << "Pipe " <<  tor << " " << _hpr + uplink 
                     << " " << (double) bytes / (LINK_RATE / 8 * timeAsSec(_period) * REPORTING_PERIOD)  << endl;
            }
        }
