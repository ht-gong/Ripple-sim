...
//NOTE: (sirius) in case of failure this will stop trying to send until next slice, which is not optimal (should send ctl pkts)
void CompositeQueue::beginService(){
    ...
    switch(_serv){
        case QUEUE_HIGH:
        {
            ...
            int finish_slice = _top->time_to_slice(eventlist().now()+drainTime(sent_pkt));
            bool in_time = (crt != _dl_queue) && crt==finish_slice;
    ...
    if(!sent_succeed) _serv = QUEUE_INVALID;
}
...
void CompositeQueue::receivePacket(Packet& _pkt) {
    ...
    //in sirius no packet can be scheduled in reconfiguration slices (odd slices)
    assert(crt == _dl_queue || crt%2==0);
...
simtime_picosec HopDelayForward::routing(Packet* pkt, simtime_picosec t) {
    ...
    */
    ...
    //needs to be calculated in advance in sirius, as link does not disappear regardless of time once sent
    int nextToR = top->get_nextToR(sent_slice, pkt->get_crtToR(), uplink);
    pkt->set_nextToR(nextToR);
    //cout << " hop " << pkt->get_crthop() << " crtToR " << pkt->get_crtToR() << " crt slice "<< slice << " sent slice " << sent_slice << " uplink " << uplink << " nexttor " << nextToR << endl;
    ...
    Queue* q = top->get_queue_tor(pkt->get_nextToR(), uplink);
    CompositeQueue *cq = dynamic_cast<CompositeQueue*>(q);
    simtime_picosec send_t = sent_slice == slice ? t : top->get_slice_start_time(sent_slice);
    int finish_slice = top->time_to_slice(send_t + cq->drainTime(pkt));
    if(sent_slice != finish_slice){
        // Miss this slice, have to wait till the next slice
        int next_absolute_slice = top->time_to_absolute_slice(send_t) + 1;
    ...
    int wait_slice = sent_slice - slice;
    // Take in consideration negative modulo
    wait_slice = wait_slice < 0 ? (top->get_nsuperslice()*2-slice)+sent_slice+1 : wait_slice;
    assert(wait_slice >= 0);
...
#ifdef CALENDAR_CLEAR
    //wrap around modulo
    int prev = ((crt-1)+_top->get_nsuperslice())%(_top->get_nsuperslice());
