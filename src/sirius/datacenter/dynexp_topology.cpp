...
pair<int, int> DynExpTopology::get_direct_routing(int srcToR, int dstToR, int slice) {
  ...
  return make_pair(get<1>(connected[index]), get<0>(connected[index]));
...
void DynExpTopology::read_params(string topfile) {
  ...
  if (input.is_open()){
    ...
    _slicetime.resize(3);
    stream >> _slicetime[0]; // time spent for slice
    stream >> _slicetime[1]; // time spent for reconfig
    ...
    for (int i = 0; i < _nslice; i++) {
      _adjacency[i].resize(_no_of_nodes);
      if(i%2 != 0){
        //odd slices are reconfiguration slices, no connections
        fill(_adjacency[i].begin(), _adjacency[i].end(), make_pair(-1,-1));
        continue;
      }
      getline(input, line);
      stringstream stream(line);
      for (int j = 0; j < _no_of_nodes; j++) {
        stream >> tor >> port;
        //destination port in sirius, as src and dst ports are different
        _adjacency[i][j] = make_pair(tor, port);
      }
    }
    ...
    for (int i = 0; i < _nslice; i++) {
      for (int j = 0; j < _nul*_ntor; j++) {
        ...
        int dst_tor = get<0>(_adjacency[i][j]);
        if(dst_tor >= 0)
            _connected_slices[src_tor][dst_tor].push_back(make_tuple(i, uplink_to_port(j), get<1>(_adjacency[i][j])));
      }
    }
    ...
    while(!input.eof()) {
      ...
      if (vtemp.size() == 1) { // entering the next topology slice
        ...
        _nexthop[s][d][slice][0] = vtemp[2]+6; //outbound port
        _nexthop[s][d][slice][1] = vtemp[3]; //inbound port
        _nexthop[s][d][slice][2] = vtemp[4]; //sending slice
      }
    }
...
int DynExpTopology::get_nextToR(int slice, int crtToR, int crtport) {
  ...
  return _adjacency[slice][uplink].first;
}
