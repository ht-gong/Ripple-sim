...
class DynExpTopology: public Topology{
  ...
  int tor_port_to_uplink(int tor, int port); // Given the tor and relative uplink, get the global uplink index
 ...
 private:
  ...
  // indexing: [slice][src uplink (indexed from 0 to _ntor*_nul-1)] -> tor,port
  vector<vector<pair<int,int>>> _adjacency;
  // Multi-hop paths
  // indexing: [src][dst][current_slice] -> dst uplink, src uplink, the slice to send it out (vector[3])
  vector<vector<vector<vector<int>>>> _nexthop;
  // Connected time slices
  // indexing: [src][dst] -> <time_slice, out-port, in-port> where the src-dst ToRs are connected
  vector<vector<vector<tuple<int, int, int>>>> _connected_slices;
