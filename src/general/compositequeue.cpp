      ...
      _alarm(QueueAlarm(eventlist, port, this, topology))
{
    ...
    int slices = topology->get_nsuperslice()*3;
...
void CompositeQueue::receivePacket(Packet& _pkt) {
    ...
    if(!pkt->get_longflow())
        pkt->set_crtslice(time_crt);
    int crt = _top->is_downlink(_port) ? _dl_queue : _pkt.get_crtslice();
    //in general make sure we did not put a short flow in a previous queue
...
void CompositeQueue::clearSliceQueue(int slice){
    //debug 
    int long_drop = _enqueued_low_prime[slice].size() + _enqueued_high_prime[slice].size();
    int short_drop = _enqueued_low[slice].size() + _enqueued_high[slice].size();
    if(long_drop + short_drop > 0)
        cout << "Calendarclear " << long_drop+short_drop << " " << long_drop << " " << short_drop << endl;
...
int CompositeQueue::next_tx_slice(int crt_slice){
    ...
    if(crt_slice == now)
        return now;
    int i = (crt_slice+1)%(_top->get_nsuperslice()*3);
    //look for next slice queue that has to send
    while (i != now){
        if (slice_queuesize(i) > 0)
            return i;
        i = (i+1)%(_top->get_nsuperslice()*3);
    }
    return now;
}
...
mem_b CompositeQueue::slice_maxqueuesize(int slice){
    ...
    if (slice == _dl_queue){
        ...
        return _ps_per_byte*_top->get_slicetime(slice%3);
    }
}
...
mem_b CompositeQueue::slice_queuesize(int slice){
    assert(slice <= _dl_queue);
    return _queuesize_low[slice]+_queuesize_low_prime[slice]
        +_queuesize_high[slice]+_queuesize_high_prime[slice];
...
void HopDelayForward::doNextEvent() {
    ...
    DynExpTopology* top = pkt->get_topology();
    Queue* nextqueue = top->get_queue_tor(pkt->get_crtToR(), pkt->get_crtport());
    pkt->set_crtslice(top->time_to_slice(eventlist().now()));
    nextqueue->receivePacket(*pkt);
...
simtime_picosec HopDelayForward::routing(Packet* pkt, simtime_picosec t) {
    ...
    if (!pkt->get_longflow()) {
        ...
        int uplink = top->get_port(pkt->get_src_ToR(), top->get_firstToR(pkt->get_dst()),
    ...
    if (sent_slice == slice) {
		...
		int slice_delta = (finish_slice - slice + top->get_nsuperslice() * 3)
			% (top->get_nsuperslice() * 3);
        ...
        int middle_slice = (slice + 1) % (top->get_nsuperslice() * 3);
		...
		int wait_slice = (sent_slice - slice + top->get_nsuperslice() * 3)
			% (top->get_nsuperslice() * 3);
...
#ifdef CALENDAR_CLEAR
    //wrap around modulo
    int prev = ((crt-1)+_top->get_nsuperslice()*3)%(_top->get_nsuperslice()*3);
