...
simtime_picosec Routing::routing(Packet* pkt, simtime_picosec t) {
	...
	int nextPort = top->get_port(pkt->get_src_ToR(), top->get_firstToR(pkt->get_dst()),
        ...
        cout <<seqno << " Routing crtToR: " << pkt->get_crtToR() << "Srctor:" <<pkt->get_src_ToR() << " dstToR: " << top->get_firstToR(pkt->get_dst()) << " slice: " << slice 
            << " hop: " << pkt->get_crthop() << " port: "<<nextPort<< " Now: " << t <<endl;
